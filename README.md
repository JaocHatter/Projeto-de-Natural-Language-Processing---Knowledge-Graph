...
The team manually annotated **413 candidates** (10 cases, two annotators — 9/10 agreement on the
overlap; offsets index the LF cleaned corpus) using `clinical-kg annotate-relations`. Against this gold set, at the measured decision
threshold (`decide.threshold=1.0`):

**P=0.602 · R=0.656 · F1=0.628** (0.772 relation-type accuracy, 0.917 assertion accuracy)
...
drops F1 from 0.628 to 0.478, -0.150) — consistent with the finding that 34% of adjacent entity
...
(1.5), F1 was 0.503 (R=0.430); at 1.0, it rises to 0.628 (R=0.656) with no meaningful loss of
precision.

All 73 automated tests in the project pass on this same checkout.
...
code suggestion was reviewed and validated against the automated test suite (73 tests) and against
the manually annotated gold set before being incorporated. The gold set itself was built with the
interactive annotator with language-model assistance: candidate judgements were proposed by
reading each candidate's sentence against the project's relation schema and then reviewed by the
team, so the evaluation numbers above should be read with that provenance in mind.
